...
```{r}
ini.parms <- c(-0.9,-0.3 ,-2.1 )
```

Fit the logistic model to data:  
```{r}
haddock <- assess(harvest,index,calc.vcov=TRUE,ini.parms)
haddock
```

Extract the maximum likelihood and parameter estimates  
```{r}
biomass.mle <- haddock$biomass
print(biomass.mle)
pars.mle <- haddock$pars
print(exp(pars.mle))
```
...
for (i in 1:niter) {
  pars.iter[i,] <- mvtnorm::rmvnorm(1, mean = haddock$pars,
                       sigma = haddock$vcov)
}
...
```{r}
control.pars <- list()
control.pars$Htarg <- 0.75*SS.Fmsy$Fmsy[3]
